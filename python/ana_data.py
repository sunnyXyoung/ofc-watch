...
for line in a:
	...
	for message in line_dict['report']['messages']['messages']:
		if message.get('s') == 'critical' and message['m'][:3] == '獲得了':
			...
			loot_count[line_dict['report']['aName']] = loot_count.get(line_dict['report']['aName'], [f"第{message['m'].split(' ')[1]}層：{message['m'].split(' ')[3]}"]) + [f"第{message['m'].split(' ')[1]}層：{message['m'].split(' ')[3]}"]
...
# print(initial_time)
# print(report_line_graph)
# print(xp_count)
# print(times_count)

logging.info(loot_index)
print(global_weapon_list)
