...
with open(os.path.join(web_root, round, 'Faction4.json'), 'w', encoding='utf8') as f:
    ...
    f.write(str([i[1] for i in sorted(faction_loot_count.items(), key=lambda x: x[1])]).replace("'", '"'))
...
with open(os.path.join(web_root, round, 'Loot.json'), 'w', encoding='utf8') as f:
    f.write(str([{'name': i[0], 'faction': player_db[i[0]]['faction'], 'times': len(i[1]), 'loots': i[1]} for i in
